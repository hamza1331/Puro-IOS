 ...
 class Report extends Component{
     ...
     handleSubmit(){
         ...
         if(this.state.title.length===0){
             Alert.alert('Ha fallado',"Ingresar titulo")
         }
        else if(this.state.description.length<10){
            Alert.alert('Ha fallado','La descripción debe tener al menos 10 caracteres de longitud.!')
         }
         else{
             let data = {
                ...
                if(data.message==='Success')
                    {
                        if(Platform.OS==='android'){
                            ToastAndroid.showWithGravityAndOffset(
                              'Solicitud Enviada, nos pondremos en contacto con usted en Breve.!!',
                              ToastAndroid.LONG,
                              ToastAndroid.BOTTOM,
                              25,
                              50,
     ...
     render() {
      ...
      },
      {
       ...
       return (
        ...
        <Text style={{fontSize:20,marginLeft:20,marginTop:20,fontWeight:'bold'}}>INFORME/SUGERENCIA</Text>
        <Icon
