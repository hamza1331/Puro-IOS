...
class Filter extends Component {
      ...
      this.props.navigation.navigate('HomeScreen')
        ...
        if(Platform.OS==='android'){
          ToastAndroid.showWithGravityAndOffset(
            `No se encontró Listado ${this.state.distance} KM cerca!!`,
            ToastAndroid.LONG,
            ToastAndroid.BOTTOM,
            25,
            50,
  ...
  callLocation(that){
      ...
      Geolocation.getCurrentPosition(info=>{
        that.setState({ longitude:info.coords.longitude,latitude:info.coords.latitude });
        this.getDataByLocation()
      });
   }
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: '#F3F3F2' }}>
        <ScrollView>
          ...
          <View style={{ marginTop: 25, borderBottomColor: 'white', borderBottomWidth: 2, paddingBottom: 10 }}>
            <Text style={{ marginLeft: 20, fontSize: 25, fontWeight: 'bold', color: '#B46617' }}>Su Presupuesto?</Text>
          ...
          <View style={{ marginTop: 25, borderBottomColor: 'white', borderBottomWidth: 2, paddingBottom: 10 }}>
            <Text style={{ marginLeft: 20, fontSize: 25, fontWeight: 'bold', color: '#B46617' }}>Otros Opciones?</Text>
            <View style={{ alignItems: 'center', justifyContent: 'space-around', flexDirection: 'row' }}>
               <View>
                <Icon name={this.state.deliverable===true?'car-pickup':'clear'}
                  type={this.state.deliverable===true?"material-community":'community'}
                  onPress={()=>{
                    this.setState({
                      deliverable:true
                    })
                  ...
                  color='#B46617' />
                <Text style={{ color: '#B46617' }}>Entregable</Text>

              </View> 
               <View>
                <Icon name={this.state.deliverable===false? 'check':'clear'}
                  type={this.state.deliverable===false?'font-awesome':'community'}
                  onPress={()=>{
                    this.setState({
                      deliverable:false
                    })
                  ...
                  color='#B46617' />
                <Text style={{ color: '#B46617' }}>Solo para recoger</Text>
              ...
              <View>
                <Icon name={this.state.trade===true?'check':'ios-git-compare'}
                  ...
                  color='#B46617' />
                <Text style={{ color: '#B46617' }}>Aceptar intercambio?</Text>

              </View>
            </View>
          </View>
          <View style={{ marginTop: 25, borderBottomColor: 'white', borderBottomWidth: 2, paddingBottom: 10 }}>
            <Text style={{ marginLeft: 20, fontSize: 25, fontWeight: 'bold', color: '#B46617' }}>Fecha de Publicación</Text>
            <View style={{ alignItems: 'center', justifyContent: 'space-around', flexDirection: 'row' }}>
              <View onTouchEnd={()=>{
                this.setState({
                  days:1,
                ...
                <Text style={{ color: '#B46617',textDecorationLine:this.state.days===1?'underline':'none',fontWeight:this.state.days===1?'bold':'normal' }}>Horas</Text>
              ...
              <View onTouchEnd={()=>{
                ...
                <Text style={{ color: '#B46617',textDecorationLine:this.state.days===7?'underline':'none',fontWeight:this.state.days===7?'bold':'normal' }}>Dias</Text>
              ...
              <View onTouchEnd={()=>{
                ...
                <Text style={{ color: '#B46617',textDecorationLine:this.state.days===30?'underline':'none',fontWeight:this.state.days===30?'bold':'normal' }}>Dias</Text>
              ...
              <View onTouchEnd={()=>{
                ...
                <Text style={{ color: '#B46617',textDecorationLine:this.state.days===90?'underline':'none',fontWeight:this.state.days===90?'bold':'normal' }}>Dias</Text>
              </View>
            </View>
          </View>
          <View style={{ marginTop: 25, borderBottomColor: 'white', borderBottomWidth: 2, paddingBottom: 10 }}>
            <Text style={{ marginLeft: 20, fontSize: 25, fontWeight: 'bold', color: '#B46617' }}>Búsqueda por Distancia</Text>
            <View style={{ alignItems: 'center', justifyContent: 'space-around', flexDirection: 'row' }}>
              <View onTouchEnd={()=>{
                this.setState({
                  distance:50
        ...
        <View style={{ display: 'flex', backgroundColor: 'white', justifyContent: 'space-between', flexDirection: 'row', position: 'absolute', left: 0, right: 0, bottom: 0, height: 60, width: '100%' }}>
          <View onTouchEnd={this.handleReset} style={{ alignItems: 'center', justifyContent: "center" }}>
            <Text style={{ fontSize: 16, fontWeight: 'bold', marginLeft: 30 }}>Restablecer Filtros</Text>
          </View>
          <View onTouchEnd={this.handleSubmit} style={{ alignItems: 'center', justifyContent: "center" }}>
            <Text style={{ fontSize: 16, fontWeight: 'bold', marginRight: 30 }}>Aplicar Filtros</Text>
          </View>
        </View>
