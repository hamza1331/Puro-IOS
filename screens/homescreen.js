...
import {
  showListingCategoriesAction,
  renderItemAction,
  addtListingsAction,
  showDescriptionModalAction,
...
class HomeScreen extends Component {
  ...
  handleCategoryClick = value => {
    ...
    fetch(url + "/api/getListings" + this.state.page, {
  ...
  async componentDidMount() {
    setTimeout(()=>{console.log('URL ',url)},5000)
    this.checkPermission();
    this.createNotificationListeners(); //add this line
    fetch(url+'/api/getUserData'+this.props.UID)
    .then(res=>res.json())
    .then(response=>{
      ...
      leftComponent={
        ...
        />
          ...
          centerComponent={
           ...
           placeholder={'Buscar Puro Artesanal'}
             shake={true}
           ...
           leftIcon={
             ...
             placeholder={`Buscar ${this.props.selectedCategory}`}
             shake={true}
             inputContainerStyle={{width:"100%",backgroundColor:'white',borderRadius:12,borderBottomColor:'white'}}
          ...
          rightComponent={
            <Text onPress={()=>this.props.navigation.navigate("Filter")} style={{fontSize:20,color:'darkred'}}>Filtrar</Text>
  ...
  {this.props.subCategories.length>0 && this.props.subCategories.map((subcat,index)=>{
    return <View style={{marginLeft:8,justifyContent:'center',alignItems:'center'}}>
    <Icon
            reverse
  ...
  <CardAction 
   ...
   <Button
       ...
       {this.state.loadingMore===true && <ActivityIndicator size={Platform.OS==='android'? 14:1} animating/>}
      ...
      marginBottom:25
            ...
            <Button title="+Agregar Listado" onPress={()=>{
              if(this.state.allowListing===false){
                Alert.alert(
                  'Advertencia',
                  'Debe completar su información de pago para recibir pagos ...',
                  [
                    {text: 'De todas maneras, continúe', onPress: () => {
                      this.props.showListingCategories()
                    }, style: 'cancel'},
                    {text: 'Si, Preparar Perfil', onPress: () => {
